...
//kan det vara det som är felet? queery läses aldrig?
...
app.post('/upload', upload.single('image_url'), async (request, response) => {
  console.log(request.file, request.files, request.body)
  console.log(upload.single('image_url'))

  const { name, description, brand, size, color, condition_comment, category_id } = request.body;

  try {
    const query = `
      INSERT INTO clothes (name, description, brand, size, color, condition_comment, category_id, image_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`;
